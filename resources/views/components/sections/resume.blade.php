<section id="resume" class="min-h-screen py-24 flex flex-col justify-center px-8 lg:px-24 bg-transparent pointer-events-none relative z-10">
    <div class="pointer-events-auto w-full max-w-7xl mx-auto gs-fade-up">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 relative overflow-hidden rounded-3xl bg-white/10 dark:bg-white/5 border border-black/10 dark:border-white/10 backdrop-blur-md p-8 md:p-10 transition-all duration-500 hover:bg-white/20 dark:hover:bg-white/10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    @foreach(__('content.resume.experience') as $job)
                        <div class="relative pl-6 border-l border-black/20 dark:border-white/20">
                            <!-- Timeline dot -->
                            <div class="absolute w-3 h-3 bg-black dark:bg-white rounded-full -left-[6.5px] top-1.5 opacity-80 shadow-[0_0_10px_rgba(255,255,255,0.5)]"></div>
                            
                            <span class="text-sm font-mono opacity-60 mb-2 block tracking-wider">{{ $job['years'] }}</span>
                            <h4 class="text-lg font-bold leading-tight mb-1">{{ $job['title'] }}</h4>
                            <span class="text-sm font-medium text-[#FF4433] block mb-3">{{ $job['company'] }}</span>
                            <p class="text-sm opacity-70 leading-relaxed">{{ $job['description'] }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <!-- Education Card -->
            <div class="lg:col-span-1 relative overflow-hidden rounded-3xl bg-white/10 dark:bg-white/5 border border-black/10 dark:border-white/10 backdrop-blur-md p-8 md:p-10 transition-all duration-500 hover:bg-white/20 dark:hover:bg-white/10">
                <h3 class="text-2xl font-bold mb-8 flex items-center gap-3">
                ...
                <div class="space-y-8">
                    @foreach(__('content.resume.education') as $edu)
                        <div class="relative">
                            <span class="text-sm font-mono opacity-60 mb-1 block">{{ $edu['year'] }}</span>
                            <h4 class="text-base font-bold leading-tight mb-1">{{ $edu['degree'] }}</h4>
                            <p class="text-sm text-[#FF4433]">{{ $edu['school'] }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <!-- Skills Card -->
            <div class="lg:col-span-3 relative overflow-hidden rounded-3xl bg-black/5 text-black dark:text-white dark:bg-black/40 border border-black/10 dark:border-white/10 backdrop-blur-md p-8 md:p-10 transition-all duration-500">
